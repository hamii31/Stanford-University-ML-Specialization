...
X_train = np.array([[952, 2, 1, 65], [1244, 3, 2, 64], [1947, 3, 2, 17], [3000, 4, 2, 30], [1345, 2, 1, 10], [1824, 1, 2, 30], [1132, 2, 2, 8]])
y_train = np.array([271.5, 232, 509.8, 910, 340, 492.5, 288.9])
X_features = ['size(sqft)','bedrooms','floors','age']

# plot each feature vs its price
...
# Bad examples
#set alpha to 9.9e-7
_, _, hist = run_gradient_descent(X_train, y_train, 10, alpha = 9.9e-7) # the cost is horrible
plot_cost_i_w(X_train, y_train, hist)

#set alpha to 9e-7
_,_,hist = run_gradient_descent(X_train, y_train, 10, alpha = 9e-7) # the cost is better, but still bad
plot_cost_i_w(X_train, y_train, hist)

# Good example
#set alpha to 1e-7
_,_,hist = run_gradient_descent(X_train, y_train, 10, alpha = 1e-7) # the cost is good
plot_cost_i_w(X_train,y_train,hist)

# Demonstrate different feature scaling methods 
...
# Re-run our gradient descent algorithm with normalized data. Note the vastly larger value of alpha. This will speed up gradient descent.
